...
class MatrizConfusion:
    def __init__(self, resultados: pd.DataFrame) -> None:
        self._resultados = resultados
        self._total = len(resultados)
        self._correcto = len(resultados[resultados['prediccion'] == resultados['clase']])
        

        clases = np.unique(np.concatenate([resultados['clase'].unique(), resultados['prediccion'].unique()]))
        self._matriz_confusion = pd.crosstab(pd.Categorical(resultados['clase'], categories=clases), 
                               pd.Categorical(resultados['prediccion'], categories=clases),
                               rownames=['actual'],
                               colnames=['prediccion'],
                               dropna=False)
        
    def __add__(self, valor: object) -> object:
        resultados_nuevos = pd.concat([self._resultados, valor._resultados])
    ...
    def verdaderos_negativos(self: object) -> pd.Series:
    ...
    def falsos_positivos(self: object) -> pd.Series:
    ...
    def falsos_negativos(self: object) -> pd.Series:
    ...
    def f_score(self, b: float) -> pd.Series:  # Notemos que: si b =1 (f1 score), se le da igual importancia a la precision y al recall; si b > 1, se le da mas importancia a la precision; si b < 1, se le da mas importancia al recall
